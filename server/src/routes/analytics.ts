...
router.get('/performance', async (req: any, res: any) => {
  try {
    ...
    // Device performance
    const devicePerformance = await pool.query(
      `
      SELECT 
        d.platform,
    ...
    const confirmationRate =
      accuracyData.total_detections > 0
...
router.get('/geography', async (req: any, res: any) => {
  try {
    ...
    const densityGrid = await pool.query(
      `
      SELECT 
    ...
    const trafficPatterns = await pool.query(
      `
      SELECT 
        traffic_light_state,
...
router.get('/realtime', async (req: any, res: any) => {
  try {
    ...
    const realtimeActivity = await pool.query(`
      SELECT 
