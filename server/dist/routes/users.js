...
router.get('/:userId/stats', async (req, res) => {
    try {
        ...
        // Get user's devices
        const userDevices = await server_1.pool.query(`
    ...
    `, [userId]);
        ...
        const stats = await server_1.pool.query(`
      SELECT 
    ...
    `, [deviceIds, interval]);
        // Get daily activity
        const dailyActivity = await server_1.pool.query(`
      SELECT 
      ...
      ORDER BY date DESC
    `, [deviceIds, interval]);
        const detectionsByState = {};
        let totalDetections = 0;
        let totalConfidence = 0;
        stats.rows.forEach((row) => {
            detectionsByState[row.traffic_light_state || 'unknown'] = parseInt(row.state_count);
